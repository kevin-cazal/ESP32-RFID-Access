...
cmd_t cmds[MAX_CMDS] = {
        {.name = "add", .fn = shell_mem_add, .help = "add name uid perm"},
        {.name = "del", .fn = shell_mem_del, .help = "del id"},
...
{
    mem_read_from_fs(&tag_db, STORAGE, args[1].c_str());
}

void shell_save(String args[MAX_ARGS])
{
    mem_write_to_fs(&tag_db, STORAGE, args[1].c_str());
}
...
void shell_save_default(String args[MAX_ARGS]) {
    mem_write_to_fs(&tag_db, STORAGE, TAG_DB_FILE);
}
...
void shell_cmd_not_found(String args[4])
{
    Serial.println("Command not found. Use 'help' to show available commands.");
}

void shell_show_mem(String args[4])
{
    mem_dump_to_serial(&tag_db);
}
